...
func (s *CommentStore) Create(ctx context.Context, cmt *Comment) error {
	query := `
	...
	if err := s.db.QueryRowContext(ctx, query, cmt.PostID, cmt.UserID, cmt.Content).Scan(&cmt.ID, &cmt.CreatedAt); err != nil {
		return err
	}
